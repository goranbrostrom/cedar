...
library(knitr)
opts_chunk$set(
fig.path='figs/',include=TRUE,cache=TRUE
)
@
...
\begin{itemize}
\item Model: $h(t; \mathbf{x}) = h_0(t) \exp{(\boldsymbol{\beta}
\mathbf{x})}$.
\item Data: $(t_{0i}, t_i, d_i, \mathbf{x}_i), \; i = 1, \ldots, n$.
\item Two components to estimate:
   \begin{itemize}
      \item The regression coefficients $\boldsymbol{\beta} = (\beta_1,
\ldots, \beta_k)$.
     \item The baseline hazard function $h_0(t)$.
   \end{itemize}
\end{itemize}
...
\begin{frame}[fragile]{Male mortality, Skellefteå 1800--1820}
\scriptsize
<<mmor,echo=TRUE>>=
library(eha)
data(male.mortality)
...
\begin{frame}{Discrete Time Methods}

\begin{itemize}
\item $t_1 < t_2 < \cdots < t_k$ possible event times.
\item $P(\mbox{event at}\; t_j \mid \mbox{no event before}; \bx) =
\psi(\alpha_i, \bbeta, \bx)$.
\item Problem: Many extra parameters to estimate
($\boldsymbol{\alpha}$).
\item Advantage: Fully parametric model; hazard function is fully
specified.
\item But what is \emp{proportional hazards in discrete time}?
\end{itemize}
\end{frame}

%-----------------------------------------------------------------------

\begin{frame}{Proportional Hazards in Discrete Time}

Assume \emp{continuous} proportional hazards, and a partition of
time: $0 < t_1 < t_2 < \cdots < t_k = \infty$. Then


\begin{eqnarray*}
P(t_{j-1} \le T < t_j \mid T \ge t_{j-1}; \; \bx) & = &
\frac{S(t_{j-1}\mid \bx) - S(t_j\mid \bx)}{S(t_{j-1} \mid \bx)} \\
& = & 1 - \frac{S(t_j \mid \bx)}{S(t_{j-1} \mid \bx)} \\
& = & 1 -
\left(\frac{S_0(t_j)}{S_0(t_{j-1})}\right)^{\exp(\bbeta\bx)} \\
&=& 1 - (1 - h_i)^{\exp(\bbeta\bx)}
\end{eqnarray*}
with $h_i = P(t_{j-1} \le T < t_j \mid T \ge t_{j-1}; \; \bx =
\mathbf{0})$,
$j = 1, \ldots, k$.
...
X = \left\{ \begin{array}{ll}
         ...
         \end{array} \right.
\end{equation*}
\item Then $P(X = j \mid X \ge j; \; \bx) =  1 - (1 -
h_i)^{\exp(\bbeta\bx)}$
\item Let $(1 - h_j) = \exp(-\exp(\alpha_j)), \; j = 1, \ldots, k$.
\item Then $P(X = j \mid X \ge j; \; \bx) =  1 - \exp(-\exp\big(\alpha_j
+ \bbeta\bx)\big)$
\item This is \emp{logistic regression} with \emp{cloglog link}!
\item If the $h$s are ``small'', can approximate with logit link.
\end{itemize}
