...
%\SweaveOpts{prefix=TRUE,prefix.string=../../figs/ch1,height=4}
...
noted.
\item Information may be \emp{incomplete} for some individuals:%Naturally, things may happen that makes it necessary to interrupt an
...
females[c(1:2, 101:103, 202:204, 303:305, 404), 1:2]
@ 
<<anda>>=
...
plot(60:99, both$risk[both$sex == "man" & both$age <= 99 & both$age >= 60], 
     type = "s", col = "blue", ylim = c(0, 0.5),
     ylab = "Age-specific mortality", xlab = "Age")
lines(60:99, both$risk[both$sex == "woman" & both$age <= 99 & both$age >= 60], 
      type = "s", col = "red")
abline(h = 0)
@ 
